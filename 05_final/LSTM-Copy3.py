...
def parse_command_line():
    ...
    parser.add_argument('-r', '--runs', type=str, default='final/LSTM5', help='tensorboard location')
    parser.add_argument('-b', '--batchsize', type=int, default=64, help='batchsize')
    parser.add_argument('-m', '--max', type=int, default=500, help='batchsize')
...
for i in range(max_epochs):
    ...
    if f1>f1_best:
        savetxt('lstm_f1_gt.csv', gt, delimiter=' ')
        savetxt('lstm_f1_pred.csv', pred, delimiter=' ')
        f1_best = f1
        PATH = "../modelfinal/lstm"+"best.pth"
        torch.save(model.state_dict(), PATH)
    if acc>acc_best:
        savetxt('lstm_acc_gt.csv', gt, delimiter=' ')
        savetxt('lstm_acc_pred.csv', pred, delimiter=' ')
        acc_best = acc
    if mpca>mp_best:
        savetxt('lstm_mp_gt.csv', gt, delimiter=' ')
        savetxt('lstm_mp_pred.csv', pred, delimiter=' ')
        mp_best = mpca    
