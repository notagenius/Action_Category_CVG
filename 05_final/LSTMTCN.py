...
def parse_command_line():
    ...
    parser.add_argument('-r', '--runs', type=str, default='final/mix2', help='tensorboard location')
    parser.add_argument('-b', '--batchsize', type=int, default=64, help='batchsize')
    parser.add_argument('-m', '--max', type=int, default=50, help='batchsize')
...
for i in range(max_epochs):
    ...
    if f1>f1_best:
        savetxt('mix_f1_gt.csv', gt, delimiter=' ')
        savetxt('mix_f1_pred.csv', pred, delimiter=' ')
        PATH = "../modelfinal/lstmtcn"+"best.pth"
        torch.save(model.state_dict(), PATH)
        f1_best = f1
    if acc>acc_best:
        savetxt('mix_acc_gt.csv', gt, delimiter=' ')
        savetxt('mix_acc_pred.csv', pred, delimiter=' ')
        acc_best = acc
    if mpca>mp_best:
        savetxt('mix_mp_gt.csv', gt, delimiter=' ')
        savetxt('mix_mp_pred.csv', pred, delimiter=' ')
        mp_best = mpca    
